...
void ex_panorama_simple() {
	cout << "Using stitcher..." << endl;
	Mat img, resize_img;
	vector<Mat> imgs;
	vector <string> name_tag;
	name_tag.push_back("./13w/left.jpg");
	name_tag.push_back("./13w/center.jpg");
	name_tag.push_back("./13w/right.jpg");

	for (int i = 0; i < 3; i++) {
		img = imread(name_tag[i], IMREAD_COLOR);
		if (img.empty()) exit(-1);
		cout << "load image " << i << endl;
		imgs.push_back(img);
	}

	//stitcher 사용한 Panorama 합성
	Mat result;
	Ptr<Stitcher> stitcher = Stitcher::create(Stitcher::PANORAMA, false);
	Stitcher::Status status = stitcher->stitch(imgs, result);

	//공통되는 영역 너무 적으면 error code 1 출력
	...
	imwrite("./13w/ex_panorama_simple.png", result);
}
...
void ex_panorama() {
	Mat matImage1 = imread("./13w/center.jpg", IMREAD_COLOR);
	Mat matImage2 = imread("./13w/left.jpg", IMREAD_COLOR);
	Mat matImage3 = imread("./13w/right.jpg", IMREAD_COLOR);
	if (matImage1.empty() || matImage2.empty() || matImage3.empty()) exit(-1);
	Mat resize1, resize2, resize3;

	resize(matImage1, resize1, Size(640, 480));
	resize(matImage2, resize2, Size(640, 480));
	resize(matImage3, resize3, Size(640, 480));
	cout << "load image and resize... " << endl;
	Mat result;
	flip(resize1, resize1, 1);
	flip(resize2, resize2, 1);
	cout << "Using my_panorama..." << endl;
	result = makePanorama(resize1, resize2, 3, 60);
	flip(result, result, 1);
	result = makePanorama(result, resize3, 3, 60);

	//imshow("ex_panorama_result", result);
	imwrite("./13w/my_panorama_result.png", result);
	waitKey();
}
