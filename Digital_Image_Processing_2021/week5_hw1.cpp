...
Mat doBPF(Mat srcImg) {
	...
	//Big size의 저주파 필터 
	Mat maskImg_B = Mat::zeros(magImg.size(), CV_32F);
	circle(maskImg_B, Point(maskImg_B.cols / 2, maskImg_B.rows / 2), 100, Scalar::all(1), -1, -1, 0);

	//small size의 저주파 필터 
	Mat maskImg_S = Mat::zeros(magImg.size(), CV_32F);
	circle(maskImg_S, Point(maskImg_S.cols / 2, maskImg_S.rows / 2), 40, Scalar::all(1), -1, -1, 0);
	
	//그 차 내부에서 band 범위 반지름 기준 100~40 사이
	Mat Mask_img = maskImg_B - maskImg_S;
