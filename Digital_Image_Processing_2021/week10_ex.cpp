...
void readImagesAndTimes(vector<Mat>& images, vector<float>& times) {
	...
	static const char* filenames[] = { "./ocv_project/img_0.033.jpg", "./ocv_project/img_0.25.jpg", "./ocv_project/img_2.5.jpg", "./ocv_project/img_15.jpg" };
...
void main() {
	...
	IdrDrago = 3 * IdrDrago;
	imwrite("Idr-Drago_.jpg", IdrDrago * 255);
	...
	imwrite("Idr-Reinhard_.jpg", IdrReinhard * 255);
	...
	imwrite("Idr-Mantiuk_1.jpg", IdrMantiuk * 255);
	cout << "save idr-Mantiuk.jpg" << endl;
	*/

	Mat gray;
	for (int i = 0; i < images.size(); i++) {
		cvtColor(images.at(i), gray, CV_BGR2GRAY);
		
		string window = "histogram_" + to_string(i);
		imshow(window, getHistogram(gray));

	}
	cvtColor(IdrDrago * 255, gray, CV_BGR2GRAY);
	imwrite("grayscale_Drago.jpg", gray);
	imshow("histogram_down", getHistogram(gray)); //히스토그램 출력
